...
class Diff {
    ...
    public static void emitDocDiffIndex(String fullReportFileName, 
        ...
        try {
            ...
            for (int i = 0; i < docDiffsArr.length; i++) {
                // TODO create the hash tables for whether changes exist
            }

            // TODO make sure the named anchors exist before creating links
            // TODO add <ul> perhaps
            ...
            for (int i = 0; i < docDiffsArr.length; i++) {
                ...
                if (diffOutput.className_ == null) {
                    ...
                    diffFile.println(link + "Package <b>" + diffOutput.pkgName_ + "</b></a><br>");
                } else {
                    if (diffOutput.id_.endsWith("!class")){
                        diffFile.println("&nbsp;&nbsp;" + link + diffOutput.className_ + "</a><br>");
                    } else {
                        // Work out what kind of member this is, and
                        // display it appropriately
                        int ctorIdx = diffOutput.id_.indexOf(".ctor");
                        if (ctorIdx != -1) {
                            diffFile.println("&nbsp;&nbsp;&nbsp;&nbsp;" + link + "Constructor " + diffOutput.id_.substring(ctorIdx + 5) + "</a><br>");
                        } else {
                            int methodIdx = diffOutput.id_.indexOf(".dmethod.");
                            if (methodIdx != -1) {
                                diffFile.println("&nbsp;&nbsp;&nbsp;&nbsp;" + link + "Method " + diffOutput.id_.substring(methodIdx + 9) + "</a><br>");
                            } else {
                                int fieldIdx = diffOutput.id_.indexOf(".field.");
                                if (fieldIdx != -1) {
                                    diffFile.println("&nbsp;&nbsp;&nbsp;&nbsp;" + link + "Field " + diffOutput.id_.substring(fieldIdx + 7) + "</a><br>");
                                }
                            }
                        }
                    }
